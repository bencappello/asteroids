...
Asteroids.Util.loadHighScores = function() {
    ...
    try {
        ...
        console.log(`Raw data from localStorage (${HIGH_SCORE_KEY}):`, storedScores); // Log raw data
        if (storedScores) {
    ...
    const finalScores = scores.slice(0, MAX_HIGH_SCORES);
    console.log("Returning scores from loadHighScores:", JSON.stringify(finalScores)); // Log final returned scores
    return finalScores;
};
...
Asteroids.Util.saveHighScores = function(scores) {
    ...
    console.log("Saving scores to localStorage:", JSON.stringify(scoresToSave)); // Log before saving
    try {
...
Asteroids.Util.isHighScore = function(score, highScores) {
    // Ensure score is a valid number before comparing
    if (typeof score !== 'number' || isNaN(score) || score <= 0) {
        console.warn(`isHighScore check failed: Invalid score provided (${score})`);
        return false;
    }
