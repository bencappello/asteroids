...
var Ufo = Asteroids.Ufo = function (obj) {
  ...
  obj.color = '#00ff00'; // Green color for UFO
  // Only generate random velocity if one isn't provided
  if (!obj.vel) {
      obj.vel = Asteroids.Util.randomVec(Ufo.SPEED);
  }
  Asteroids.movingObject.call(this, obj);
  this.type = 'Ufo'; // Override type
  this.img = new Image();
...
Ufo.prototype.collideWith = function (otherObject) {
  ...
  if (otherObject.type === 'Ship' && !otherObject.suspended) {
    ...
    var explosion = new Asteroids.AsteroidExplosion({ pos: this.pos, radius: this.radius, game: this.game });
    this.game.add(explosion); // Add first
    explosion.explode();      // Then start animation
    this.game.remove(this);
  ...
  // UFOs ignore collisions with UFO bullets (type 'UfoBullet') and other UFOs (type 'Ufo') for now
};
