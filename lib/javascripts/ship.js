...
var Ship = Asteroids.Ship = function (obj) {
  ...
  obj.vel = [0,0];
  obj.color = '#ffffff'; // White color for Ship
  Asteroids.movingObject.call(this, obj);
  this.type = 'Ship'; // Override type
  this.img = new Image();
  ...
  this.invincibleTimer = 0; // Timer in frames
  this.INVINCIBILITY_DURATION_SECONDS = 2;
};
...
Ship.prototype.draw = function (ctx) {
  ...
  if (!this.suspended) {
    ...
    // Add flashing effect if invincible
    if (this.isInvincible && Math.floor(this.invincibleTimer / 40) % 2 === 0) { // Flash every 15 frames (was 5)
