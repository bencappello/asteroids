...
// Method to check if ship is currently invulnerable (e.g., after respawn)
Ship.prototype.invulnerable = function() {
    // For now, ship is never invulnerable, but can add logic later (e.g., timer)
    return false; 
};

// Method called when ship is hit by an asteroid
Ship.prototype.hitByAsteroid = function() {
    if (this.invulnerable()) return;
    console.log("Ship hit by Asteroid!");
    this.suspendSelf(); // Suspend ship controls
    this.game.handleDeath(); // Let the game handle the death sequence
};

// Method called when ship is hit by a UFO or UFO bullet
Ship.prototype.hitByUfo = function() {
    if (this.invulnerable()) return;
    console.log("Ship hit by UFO!");
    this.suspendSelf(); // Suspend ship controls
    this.game.handleDeath(); // Let the game handle the death sequence
};
