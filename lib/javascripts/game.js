...
Game.prototype.newLife = function () {
  ...
  if (lives <= 0) {
    // GAME OVER
    this.gameOver = true;
    // Don't immediately explode/show game over, wait for initials if needed
    const highScores = Asteroids.Util.loadHighScores();
    const isNewHighScore = Asteroids.Util.isHighScore(score, highScores);
    ...
    if (isNewHighScore) {
        ...
        // The actual saving happens in the submit handler (added to index.html)
    } else {
        // No high score, proceed directly to game over screen after explosion
        console.log("Game over, score did not qualify for high scores.");
        this.ship_explosion.explode(function() {
            Asteroids.Util.displayHighScores('high-scores-gameover');
            $('#game-over').removeClass('hide');
        }.bind(this), this.ship.pos);
    }
  } else {
    // Not game over, just reset for new life
...
Game.prototype.step = function () {
  ...
  if (this.gameOver) {
    if (this.ship_explosion.frame > this.ship_explosion.img_obj.total_frames && !this.gameOverScreenShown) {
      ...
      const highScores = Asteroids.Util.loadHighScores();
      // Use the stored finalScore for the check
      console.log(`Checking if score ${this.finalScore} is high score against list:`, JSON.stringify(highScores)); // Log inputs to isHighScore
...
Game.prototype.handleDeath = function () {
  var lives = this.decrementLives();
  var currentGlobalScore = score; // Correct: Read from the global score variable
  ...
  if (lives <= 0) {
    ...
    if (!this.gameOver) { // Ensure we only trigger this once
      ...
      // Store the final score for high score check later, using the global score
      this.finalScore = currentGlobalScore;
      console.log(`Stored finalScore: ${this.finalScore}`); // Add log to confirm
    }
  } else {
