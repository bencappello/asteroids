...
var Game = Asteroids.Game = function(width, height) {
  ...
  this.startGameHandler = function() {
      ...
      this.min_asteroid_speed = this.base_min_asteroid_speed;
      lives = Game.NUM_LIVES; // Use constant
...
Game.prototype.decrementLives = function() {
    if (lives > 0) { // Ensure lives don't go negative visually
        lives -= 1;
    }
    $('#lives').html(lives); // Update the display
    console.log("Life decremented. Remaining lives:", lives);
    return lives; // Return the new number of lives
};
