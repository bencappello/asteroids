...
Game.prototype.newLife = function () {
  ...
  if (lives <= 0) {
    ...
    this.ship_explosion.explode(function() {
        // Check for high score *after* explosion animation (optional, simple check here)
        const highScores = Asteroids.Util.loadHighScores();
        if (Asteroids.Util.isHighScore(score, highScores)) {
            // Prompt for initials
            let initials = prompt(`Game Over! Score: ${score}\nEnter your initials (3 chars max) for the high score list:`, "AAA");
            if (initials) { // Add score if initials provided
                Asteroids.Util.addHighScore(initials, score, highScores);
            } else {
                 console.log("High score entry cancelled.");
            }
        }
        // Always display scores on game over screen
