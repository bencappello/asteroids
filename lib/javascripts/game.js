...
var Game = Asteroids.Game = function(width, height) {
  this.width = width || Game.DIM_X;
  this.height = height || Game.DIM_Y;
  ...
  this.base_num_asteroids = 10; // Starting number for level 1
  ...
  this.ufosSpawnedThisLevel = 0; // Add counter for UFOs spawned in the current level
...
Game.UFO_SPAWN_INTERVAL_SECONDS = 1; // Spawn UFO every 3 seconds
...
Game.DIM_X = 1000;
Game.DIM_Y = 600;
Game.FPS = 30; // Define FPS first
Game.FRAME_RATE = 1000 / Game.FPS; // Now calculate FRAME_RATE (approx 33.33ms)
Game.NUM_ASTEROIDS = 10;
Game.MAX_ASTEROIDS = 12; // Max asteroids on screen at once
Game.ASTEROID_SPAWN_RATE = 1500; // Milliseconds between asteroid spawns
Game.MAX_LEVEL = 10; // Example max level
Game.START_DELAY_SECONDS = 2; // Delay in seconds before gameplay starts
Game.INVINCIBILITY_SECONDS = 2; // Ship invincibility duration after reset
...
/**
 * Checks if spawning of new game elements (like UFOs) is currently allowed.
 * Spawning is allowed if the level start time is null (indicating attract mode or pre-game state)
 * or if the level is 2 or higher and the initial start delay has passed.
 * @returns {boolean} True if spawning is allowed, false otherwise.
 */
Game.prototype.isSpawningAllowed = function () {
    // If levelStartTime is null, assume spawning is allowed (e.g., attract mode setup)
    if (this.levelStartTime === null) {
        return true;
    }

    const elapsedSeconds = (Date.now() - this.levelStartTime) / 1000;
    return elapsedSeconds >= Game.START_DELAY_SECONDS;
};
...
Game.prototype.reset = function () {
  ...
  if (this.ship) {
    ...
    this.ship.isCollided = false; // Reset collision state

    // Make invincible
    this.ship.isInvincible = true;
    const invincibilitySeconds = Game.INVINCIBILITY_SECONDS; // Use local var for logging
    const frameRate = Game.FRAME_RATE; // Use local var for logging
    const calculatedFrames = this.secondsToFrames(invincibilitySeconds);
    // console.log(`RESET: Game.INVINCIBILITY_SECONDS = ${invincibilitySeconds}`); // REMOVED log
    // console.log(`RESET: Game.FRAME_RATE = ${frameRate}`); // REMOVED log
    // console.log(`RESET: Calculated secondsToFrames(${invincibilitySeconds}) = ${calculatedFrames}`); // REMOVED log
    this.ship.invincibleTimer = calculatedFrames;
    console.log(`Ship invincible for ${this.ship.invincibleTimer} frames`); // Restore original log
...
Game.prototype.step = function () {
  ...
  const maxUfosForLevel = this.getMaxUfosForLevel(this.level);
  if (this.asteroids.length === 0 && 
      this.ufos.length === 0 && 
      this.ufosSpawnedThisLevel >= maxUfosForLevel &&
      !this.preLevelState && // Ensure not in countdown
      this.levelStartTime !== null) { // Ensure level has actually started
      console.log(`Level ${this.level} complete. Asteroids: 0, UFOs: 0, Spawned UFOs: ${this.ufosSpawnedThisLevel}/${maxUfosForLevel}`);
      this.advanceLevel();
  } else {
      // Spawning is generally allowed if the level is active
      if (this.isSpawningAllowed()) {
          // Check specifically if UFOs should be spawned (within the level limit)
          if (this.ufosSpawnedThisLevel < maxUfosForLevel) {
              this.maybeAddUfo();
          }
          // Power-ups can spawn independently of the UFO count
...
Game.prototype.getMaxUfosForLevel = function(level) {
    ...
    if (level <= 7) {
        return baseMax + (Math.max(0, level - 2) * 1);
    } else {
  ...
  if (this.isSpawningAllowed()) {
    ...
    if (this.ufoSpawnTimer <= 0) {
      ...
      if (this.ufos.length < maxUfos) {
        ...
        this.add(ufo);
        this.ufosSpawnedThisLevel++;
      } else {
         console.log(`Max UFOs reached (${this.ufos.length}/${maxUfos}), not spawning, resetting timer.`);
      }
      // Reset timer AFTER checking/spawning
      this.ufoSpawnTimer = this.secondsToFrames(Game.UFO_SPAWN_INTERVAL_SECONDS);
      console.log("Resetting ufoSpawnTimer to:", this.ufoSpawnTimer);
    }
  }
};
...
Game.prototype.startLevelGameplay = function() {
    ...
    this.ufosSpawnedThisLevel = 0; // Reset UFO counter for the new level
