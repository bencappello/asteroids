...
var Game = Asteroids.Game = function(width, height) {
  ...
  this.base_num_asteroids = Game.EASY_ASTEROID_MODE ? 1 : 8; // Starting number for level 1
  this.base_min_asteroid_speed = Game.EASY_ASTEROID_MODE ? 0.5 : 0.5; // Reverted to original base speed
  ...
  this.currentUfoSpawnRate = Game.BASE_UFO_SPAWN_RATE; // Initialize dynamic rate
  ...
  this.startGameHandler = function() {
      ...
      this.num_asteroids = this.base_num_asteroids; // Use base values for level 1
      this.min_asteroid_speed = this.base_min_asteroid_speed;
      lives = 5; // Reset lives/score
      score = 0;
      $('#lives').html(lives);
      $('#score').html(score);
...
Game.BASE_UFO_SPAWN_RATE = Game.EASY_UFO_MODE ? 0 : 0.001; // Base rate for level 1
Game.UFO_SPAWN_INCREMENT = 0.0005; // Increment per level after level 1
...
Game.prototype.newGame = function () {
  ...
  this.bullets = [];
  this.asteroid_explosions = [];
  this.ship_explosion.reset(); // Reset ship explosion animation
  score = 0;
  lives = 5;
  $('#score').html(score);
  $('#lives').html(lives);
...
Game.prototype.addInitialAsteroids = function (isAttractMode) {
  var new_asteroids = [];
  var num = isAttractMode ? 5 : this.num_asteroids; // Fewer asteroids for attract mode
...
Game.prototype.step = function () {
  ...
  if (this.attractMode) {
      this.moveObjects();
      // We might still need to draw in the main loop? Let's see.
      // GameView loop handles drawing based on currentGame.draw
      return;
  }
...
Game.prototype.maybeAddUfo = function () {
  // Only add UFO if spawning is allowed, probability met, AND level is 2 or higher
  if (this.level >= 2 && this.isSpawningAllowed() && Math.random() < this.currentUfoSpawnRate) {
...
Game.prototype.advanceLevel = function() {
    this.level++;
    $('#level').html(this.level);
    // Optional: Adjust difficulty parameters based on new level
    this.num_asteroids = this.base_num_asteroids + (this.level - 1); // Example: +1 asteroid per level
    this.min_asteroid_speed = this.base_min_asteroid_speed + (this.level - 1) * 0.1;
    // Update dynamic UFO spawn rate
    this.currentUfoSpawnRate = Game.BASE_UFO_SPAWN_RATE + (this.level - 1) * Game.UFO_SPAWN_INCREMENT;

    console.log("Advancing to level: ", this.level);
    console.log("Updated UFO Spawn Rate: ", this.currentUfoSpawnRate);
    this.startPreLevelSequence();
};
